...
//ponemos throws Exception en todos estos? Al getSaldoActualizadoCliente me obligó a agregarla
    private double calcularComisionChofer(Chofer chof, Calendar i, Calendar f) throws Exception {
        ...
        double facturacion  = dao.obtenerFacturacionChofer(chof, i, f);
        double comision = chof.getComision();
        comisionCalculada = facturacion * comision;
        return comisionCalculada;
    }

    public double calcularSueldo(Empleado emp,Calendar i,Calendar f) throws Exception {
        ...
        if (emp instanceof Operador) {
            ...
            sueldoCalculado = chof.getSueldo() + this.calcularComisionChofer(chof, i, f);
        }
